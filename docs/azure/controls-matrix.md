...
| `CKV_AZURE_36` | The state firewall deliberately does **not** grant every trusted Microsoft service a bypass. It accepts only the explicit runner subnet and owner break-glass IP while private access is introduced. | A specific required Azure service is approved for data-plane access. |
