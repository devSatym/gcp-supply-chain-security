...
| --- | --- | --- | --- | --- | --- | --- | --- |
| CI-001 | Provenance verification | Verification must fail when the expected SLSA predicate, source repository, builder, and signing workflow entrypoint do not match. | Verify calls Cosign with the expected signer and slsaprovenance type, then prints predicateType, builder, and entryPoint. It does not assert builder, entrypoint, source URI, or subject digest. Kyverno does assert these at admission. | Yes | HIGH | Add fail-closed jq checks in verify.yml for predicate type, builder, entrypoint, canonical source URI, and image subject digest. | Static workflow validation; policy tests; real main Verify job after merge. |
| CI-002 | Trusted manual dispatch scope | Trusted GAR/WIF/signing work must originate from main, not an arbitrary selected branch. | Deploy workflow_dispatch checks only a typed confirm value. The live WIF provider scopes repository, not ref. A writer could manually select another branch and invoke GCP Auth before the later signer identity mismatch causes Verify to fail. | Yes | HIGH | Require github.ref to equal refs/heads/main for privileged Build and Push and SBOM/VEX caller jobs. Their dependent sign/verify jobs then remain unreachable off-main. | Static event-condition review; actionlint; PR confirms no production job runs; main dispatch/push run confirms trusted path. |
...
| CI-012 | Documentation digest accuracy | Documentation should quote the actual deployed immutable digest. | README, docs/codex/01-IMPLEMENTATION-PLAN.md, docs/codex/05-HANDOFF.md, and docs/my-validation/README.md omit two hexadecimal characters from the current digest. Executable Helm values and policy fixtures use the correct digest. | Yes | LOW | Correct only the affected documentation values; do not change the Helm pin. The user-modified validation file must be reconciled separately before editing it. | Compare documentation with GAR describe and values.yaml. |
...
- CI-001 is HIGH because Verify is a stated security boundary and must not
  present a successful verification result while merely displaying an
  unexpected provenance payload.
- CI-002 is HIGH because repository-only WIF scoping combined with a
  branch-selectable manual run expands trusted cloud authority beyond intended
  main code. It does not currently create an admissible main signer identity,
...
1. Implement CI-001 and CI-002 in an additive workflow hardening commit.
2. Validate YAML/action syntax, Terraform/Helm/policy checks, and the current
   real PR gates.
3. Push the branch and open a new harmless relevant PR. Confirm Semgrep,
   Trivy, policy tests, and local PR Image Scan pass while privileged jobs skip.
4. Merge only after PR evidence is captured, then observe a real main run for
   Build and Push, Sign and Attest, and Verify.
5. Correct CI-012 documentation after the live validation. Do not overwrite
   the user-modified docs/codex/03-VALIDATION.md; merge its captured evidence
   deliberately.
6. Stop for explicit approval before CI-004 branch-protection changes.
7. Treat CI-003 as a follow-up hardening project: it needs a reviewed scanner
