...
| CI-007 | PR workflow clarity | PR should validate code without producing trusted production artifacts. | PR Image Scan now lives in PR Check; Deploy is triggered only by main pushes or manual dispatch. A relevant PR shows only applicable unprivileged gates. | No | INFO | Keep the separation. Do not make privileged jobs run on a PR. | Screenshot PR shows all applicable checks passing with no skipped Deploy jobs. |
...
| CI-010 | Root CI permissions | PR jobs should avoid write-capable repository/cloud credentials; main jobs should use least privilege. | No write-all, contents write, packages write, pull_request_target, or GCP auth in PR code path. PR Image Scan inherits PR Check's no-id-token permissions. Main build/sign/verify need id-token and registry access. | No | INFO | Keep current least-privilege split and pinned actions. | Source review and PR run job list. |
