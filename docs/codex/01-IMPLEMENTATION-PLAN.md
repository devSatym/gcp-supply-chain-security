...
Status: Phases 0–1 passed; Phase 2 Terraform/CI adaptation validates locally. The canonical topology is documented in [the audit](00-REPO-AUDIT.md) and [`docs/repository-merge.md`](../repository-merge.md). Implementation proceeds additively from the current `main` history.
...
flowchart TD
  ...
  B --> N[Apply GCP foundation + VPC + GKE]
  N --> C[Get credentials and install cluster components]
  C --> P[Configure GitHub variables and obtain trusted digest]
...
| 2. Terraform readiness | Make `infrastructure/environments/prod` safe for the intended project. **Local adaptation and validation pass.** | Parameterized backend; added GAR, required services, dedicated CI WIF/GSA, and Falcosidekick WI binding; disabled ExternalDNS/Gatekeeper/Ratify deployment by default; pinned compatible provider majors. | `terraform fmt`, backend-disabled `init` and `validate` pass locally; plan only after backend/project approval. | Plan must have explained add/change/destroy counts. Failure: provider/API/IAM error; rollback: revert un-applied config. | Project, billing, region, state-bucket choice required. NAT, GKE, nodes, and logging cost money. |
...
| 5. CI WIF and artifact path | Enable keyless GitHub-to-GCP authentication and immutable delivery. | Apply the declared repo-scoped GAR/WIF/CI-SA resources; configure GitHub variables from Terraform outputs. Workflows use unset variables now, so they cannot accidentally push to the imported GAR project. | Terraform/GitHub variable commands; push a normal branch/PR. | CI exchanges OIDC token without a JSON key, pushes SHA-tagged image, and records digest. Failure: issuer/attribute/IAM mismatch; rollback: remove newly created bindings/resources. | GitHub auth and GCP IAM authority required; GAR storage/network cost. |
