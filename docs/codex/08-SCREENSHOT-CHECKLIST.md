...
| File | Capture | Status |
| --- | --- | --- |
| `01-pr-security-gates.png` | PR #2 (or a later relevant PR): PR title, target `main`, Detect relevant changes, Semgrep, Trivy, Policy Unit Tests, and PR Image Scan all PASS | Ready to capture |
...
It is acceptable for the main-only Build and Push, SBOM/VEX, Sign and Attest,
and Verify jobs to be shown as skipped on the PR screenshot. The screenshot
must make clear that the PR image scan passed; the skipped jobs intentionally
remain unavailable to untrusted PR code.
